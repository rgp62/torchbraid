...
def train_epoch(rank, model, pde, optimizer, epoch, criterion, criterion_kwargs, compose, log_interval, mgopt_printlevel):
  ...
  start_time = timer()
  optimizer.zero_grad()
  output = pde.loss(model)

  loss = compose(criterion, output, torch.zeros(()), **criterion_kwargs)
  loss.backward()
  stop_time = timer()
  optimizer.step()
  ...
  root_print(rank, mgopt_printlevel, 1, '  Train Epoch: {} \tLoss: {:.6f}\tTime Per Batch {:.6f}'.format(
      epoch, loss.item(), total_time))

def test(rank, model, pde, criterion, criterion_kwargs, compose, mgopt_printlevel, indent=''):
  ...
  total_time += stop_time-start_time
  root_print(rank, mgopt_printlevel, 1, '  Loss: {:.6f}\tTime Per Batch {:.6f}'.format(
      loss.item(), total_time))
...
##
# Basic TorchBraid mse loss function extended to take MG/Opt Term
def tb_mgopt_mse(output, target, network_parameters=None, v=None):
  '''
  Define mse entropy loss with optional new MG/Opt term
  '''
  # Use PyTorch's loss, which should be identical
  criterion = nn.MSELoss()
  loss = criterion(output, torch.zeros(()))
...
class mgopt_solver:
  ...
  def initialize_with_nested_iteration(self, ni_steps, 
    ...
    for k, steps in enumerate(ni_steps):
      ...
      for epoch in range(1, epochs + 1):
        ...
        train_epoch(rank, model, pde, optimizer, epoch, criterion, criterion_kwargs, compose, log_interval, mgopt_printlevel)
        end_time = timer()
        epoch_times.append( end_time-start_time )
    
        # test() is designed to be general for PyTorch networks
        start_time = timer()
        test(rank, model, pde, criterion, criterion_kwargs, compose, mgopt_printlevel, indent='\n  ')
        end_time = timer()
  ...
  def mgopt_solve(self, pde, 
    ...
    for epoch in range(1, epochs + 1):
      ...
      start = timer()
      test(rank, self.levels[0].model, pde, criterion, criterion_kwargs, compose, mgopt_printlevel, indent='\n  ')
      end = timer()
      ...
      if(mgopt_printlevel >= 2):
        for i, level in enumerate(self.levels[1:]):
          ...
          test(rank, level.model, pde, criterion, criterion_kwargs, compose, mgopt_printlevel, indent='    ')
  ...
  def process_criterion(self, option, model):
    ...
    if method == "tb_mgopt_mse":
      criterion = tb_mgopt_mse
